...
exec(open('config.py').read())      # config.py     WIFI 网络设置
...
# PIT俯仰角  ROL滚转轴  X：X轴位置  R_H：高度    初始值及设定目标值
PIT_S = 0;    ROL_S = 0;    X_S = 0;    R_H=abs(init_y)
PIT_goal = 0; ROL_goal = 0; X_goal = 0; H_goal=100

# web摇杆控制 ： spd速度 (L左转向 R右转向，取值：1,0,-1)
spd = 0; L = 0; R = 0

init_case=0

key_stab = False # 自稳定模式（静态稳定）
gait_mode = 0  # 步态模式 ：0 -> walk   ;  1 -> tort
stop_run_node=0 # 停止运行
...
# 舵机输出  RA_SERVO.angle(pin, 舵机初始中位（标定值） + 设置值)
...
def mainloop():
    ...
    # 判断步态模式
    if gait_mode == 0: # 步态0：trot
        if t>=(Ts-speed): # 一个完整的运动周期结束
            t=0
        elif L==0 and R==0:
            t=0
        else:
    ...
    elif gait_mode==1: # 步态1：walk
        if t>=(Ts*2+0.5-walk_speed):  # 一个完整的运动周期结束
            t=0
        elif L==0 and R==0:
            t=0
        else:
