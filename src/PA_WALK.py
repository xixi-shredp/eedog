...
# 中间变量设定
x1_s, x2_s, x3_s, x4_s = 0, 0, 0, 0
y1_s, y2_s, y3_s, y4_s = 0, 0, 0, 0
cg_adjust = 0
xs = 0
faai = 0.5
Ts = 1
...
# 计算重心偏移量函数,使得机器人重心投影始终落在三角形面


def cal_adjust(CG_Y, l, xk, sita, period):
    # 计算高度暂时固定在典型的110,1.5是角度P环值
    result = (CG_Y+period*(l+xk)/4+110*tan(sita*1.5))
...
def cal_w(CG_X, CG_Y, l, xf, h, speed, t, r1, r4, r2, r3):  # WALK步态主计算函数，相序 1-2-3-4
    ...
    # 获得陀螺仪数据
    ay = acc.get_values()
    q = PA_IMU.IMUupdate(ay["GyX"]/65.5*0.0174533, ay["GyY"]/65.5*0.0174533,
                         ay["GyZ"]/65.5*0.0174533, ay["AcX"]/8192, ay["AcY"]/8192, ay["AcZ"]/8192)
    try:
    ...
    if t < Ts*faai:  # 迈出腿1
        # print("腿1")
        padog.gesture(0, CG_X, cal_adjust(
            CG_Y, 0, l, gyro_p*pi/180, -1))  # 粗调重心
        if abs(padog.X_S-padog.X_goal) < 3:
            padog.gesture(0, CG_X, cal_adjust(
                CG_Y, 0, l, gyro_p*pi/180, -1))  # 微调重心
